...
knn_keyword[["control"]][["method"]]   # cross-validation
knn_keyword[["control"]][["number"]]   # 10-fold cross validation
knn_keyword[["control"]][["search"]]   #the author used "grid" search in its KNN model
...
mlp_keyword[["control"]][["method"]]   # cross-validation
mlp_keyword[["control"]][["number"]]   # 10-fold cross validation
mlp_keyword[["control"]][["search"]]   #the author used "random" search in its mlp model
...
glm_keyword[["control"]][["method"]]   # cross-validation
glm_keyword[["control"]][["number"]]   # 10-fold cross validation
glm_keyword[["control"]][["search"]]   #the author used "grid" search in its glm model

#down sampling applied to "okc_train" dataset to balance the frequency of "stem" and "other" in "Class" variable
levels(okc_train$Class)
table(okc_train$Class)  #before down-sampling
glm_keyword[["control"]][["sampling"]][["name"]]  
glm0 <- glm_keyword$pred[,c("obs", "Resample")]
glm1 <- glm0[glm0$Resample == "Fold01","obs"]
table(glm1)
