...
# Define training control
# in this example, we'll work with the "chicago" time series and apply resampling techniques
# we will use for simplicity's sake, cross-validation, montecarlo cv and bootstraps 
# even if we're dealing with a dataset with a data set with a temporal component
# you could use the "ames" dataset and use as predictor feature "Sale_Price"
...
load(file11)
#training <- training[1:200, ]   #comment this line to use the entire dataset
set.seed(123) 
model_cv10 <- train(form = s_40380 ~ ., 
                    data = training[, c("s_40380", 
                                        var_sets$dates)], 
                          method = "lm", 
                          metric = "RMSE", 
                          maximize = FALSE, 
...
resid_cv10 <- model_cv10[["pred"]][["pred"]] #- model_cv10[["pred"]][["pred"]]
resid_cv5 <- model_cv5[["pred"]][["pred"]] #- model_cv5[["pred"]][["pred"]]
resid_cv10_repeated10 <- model_cv10_repeated10[["pred"]][["pred"]] #- model_cv10_repeated10[["pred"]][["pred"]]
resid_cv10_repeated5 <- model_cv10_repeated5[["pred"]][["pred"]] #- model_cv10_repeated5[["pred"]][["pred"]]
resid_cv5_repeated5 <- model_cv5_repeated5[["pred"]][["pred"]] #- model_cv5_repeated5[["pred"]][["pred"]]
resid_mc <- model_mc[["pred"]][["pred"]] #- model_mc[["pred"]][["pred"]]
resid_bootstraps <- model_bootstraps[["pred"]][["pred"]] #- model_bootstraps[["pred"]][["pred"]]


d1 <- density(resid_cv10) # returns the density data 
d2 <- density(resid_cv5) # returns the density data 
d3 <- density(resid_cv10_repeated10) # returns the density data 
d4 <- density(resid_cv10_repeated5) # returns the density data 
d5 <- density(resid_cv5_repeated5) # returns the density data 
d6 <- density(resid_mc) # returns the density data 
d7 <- density(resid_bootstraps) # returns the density data 


plot(d1, main = models_vector[1])
plot(d2, main = models_vector[2])
plot(d3, main = models_vector[3])
plot(d4, main = models_vector[4])
plot(d5, main = models_vector[5])
plot(d6, main = models_vector[6])
plot(d7, main = models_vector[7])

par(mfrow=c(2,2))
x0 <- c(15,20)    #zoom density function of residuals
y0 <- c(0,0.25)
xlab0 <- "Residuals"
plot(d2, main = models_vector[2], 
     xlab = xlab0
...
list000 <- data.frame(model_cv10[["pred"]][["obs"]])
names(list000) <- "obs"
list000 <- list000[which(list000$obs >= 15 & list000$obs <= 20),]
mean(list000)
...
set.seed(123) 
ctrl_cv10_pre_pca <- trainControl(method = "cv", 
                              number = 10, 
...
set.seed(123) 
model_cv10_pre_pca <- train(form = s_40380 ~ .,
                        data = training[, c("s_40380", 
                                            var_sets$dates)],
                        method = "glm", 
                        metric = "RMSE", 
...
model_cv10_pre_pca
model_cv10_pre_pca$preProcess$rotation 
