...
# regression residuals - shapiro-wilk test 
resid1 = lm_date_only$pred$obs - lm_date_only$pred$pred
shapiro.test(resid1)  #the regression residuals are not normally distributed


# concordance correlation coefficient (CCC)
CCC_obs_pred = ccc_vec(lm_date_only$pred$obs, lm_date_only$pred$pred)
print(c("concordance correlation coefficient:", round(CCC_obs_pred, 3)))
print(c("R-squared:", round(lm_date_only$results$Rsquared, 3)))
